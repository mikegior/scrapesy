...
# scrapesy-wd-http.py
# This Python script is a deamon that will continuously monitor the '/var/www/html/uploads/' directory
# for new files uploaded by Scrapesy WebUI users. When new files are discovered, this daemon will call
...
logging.basicConfig(filename="/opt/scrapesy/logs/scrapesy-watchdog-http.log",
                    level=logging.DEBUG,
                    format='%(asctime)s - %(levelname)s: %(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S')

def on_created(event):
    #Notify and log when a new file is detected; wait 10 seconds to allow the file to finish downloading via 'scrapesy.py'
    ...
    #If sent to 'CredsToES.py' wait 10 seconds after, and remove file from '/var/www/html/uploads'
    ...
    if size == 0:
        ...
        time.sleep(10)
        logging.info(f"Removing {event.src_path} from Scrapesy WebUI uploads directory...")
